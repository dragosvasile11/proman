...
function boardBuilder(board) {
    return `<section class="board" section-board-id=${board.id}>
                <div class="board-header" data-board-id=${board.id}><span class="board-title"><p id=${board.id} class="boardTitle" name="titleEdit" contenteditable="false" onkeypress="return (this.innerText.length <= 12)">${board.title}</p></span>
...
function cardBuilder(card) {
    return `<div class="card" data-card-order="${card.card_order}" data-card-id="${card.id}" data-board-id="${card.board_id}">
                <span class="card-title" id=${card.id} name="titleEdit" contenteditable="false" onkeypress="return (this.innerText.length <= 12)">${card.title}</span>
...
function statusBuilder(status, boardId) {
    return `<div  align="center" class="board-column" data-status-id="${status.id}">
                <span class="board-column-title" data-status-id="${status.id}" id=${status.id} name="titleEdit" contenteditable="false" onkeypress="return (this.innerText.length <= 12)">${status.title}</span>
